...
test('sayHelloAsync 404', () => {
  fetchMock.get(helloEndpointRoute(404), 404)
  const store = mockStore()
  return store.dispatch(sayHelloAsync(404))
    .then(() => {
      expect(store.getActions()).toEqual([
        sayHelloAsyncRequest(),
        sayHelloAsyncFailure(),
...
test('sayHelloAsync data error', () => {
  fetchMock.get(helloEndpointRoute(400), {})
  const store = mockStore()
  return store.dispatch(sayHelloAsync(400))
    .then(() => {
      expect(store.getActions()).toEqual([
        sayHelloAsyncRequest(),
        sayHelloAsyncFailure(),
